...
//A box obstacle
ArrayList<Vec2> boxes = new ArrayList<Vec2>();
float boxW = 100;
float boxH = 250;
int movingBox;
...
// object dragging code
boolean mouseDragging = false;
Vec2 lastMousePos = new Vec2(0,0);
...
void setup(){
  size(1024,768);
  boxes.add(new Vec2(100,100));
  placeRandomObstacles();
  buildPRM(circlePos, circleRad, boxes, boxW, boxH);
...
void handleObjectMove() {
  Vec2 currMousePos = new Vec2(mouseX, mouseY);
  Vec2 direction = lastMousePos.minus(currMousePos);
  boxes.get(movingBox).subtract(direction);
...
void mouseDragged() {
  ...
  if (!mouseDragging) {
    for (int i = 0; i < boxes.size(); i++) {
      if (pointInBox(boxes.get(i), boxW, boxH, new Vec2(mouseX, mouseY))) {
        movingBox = i;
        mouseDragging = true;
...
int closestNode(Vec2 point){
  ...
  int minNode = 0;
  for (int i = 0; i < numNodes; i++) {
    if (point.distanceTo(nodePos[i]) < minDist) {
        minDist = point.distanceTo(nodePos[i]);
        minNode = i;
    }
  }
  return minNode;
...
boolean pointInBoxList(ArrayList<Vec2> boxes, float boxW, float boxH, Vec2 pointPos){
  // Return true if the point is actually inside the box
  for (Vec2 boxTopLeft : boxes) {
    if (pointInBox(boxTopLeft, boxW, boxH, pointPos)) {
      return true;
    }
  }
  return false;
...
hitInfo rayBoxListIntersect(ArrayList<Vec2> boxes, float boxW, float boxH, Vec2 ray_start, Vec2 ray_dir, float max_t) {
  hitInfo hit = new hitInfo();
  hit.t = max_t;
  for (Vec2 boxTopLeft : boxes) {
    hitInfo boxHit = rayBoxIntersect(boxTopLeft, boxW, boxH, ray_start, ray_dir, max_t);
...
void generateRandomNodes(Vec2[] circleCenters, float[] circleRadii, ArrayList<Vec2> boxes, float boxW, float boxH){
  for (int i = 0; i < numNodes; i++){
    Vec2 randPos = new Vec2(random(width),random(height));
    boolean insideAnyCircle = pointInCircleList(circleCenters,circleRadii,randPos);
    boolean insideAnyRect = pointInBoxList(boxes, boxW, boxH, randPos);
    while (insideAnyCircle || insideAnyRect){
      randPos = new Vec2(random(width),random(height));
      insideAnyCircle = pointInCircleList(circleCenters,circleRadii,randPos);
      insideAnyRect = pointInBoxList(boxes, boxW, boxH, randPos);
    }
    nodePos[i] = randPos;
  }
}
...
void connectNeighbors(){
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      if (distBetween < maxNodeDist) {
        hitInfo circleListCheck = rayCircleListIntesect(circlePos, circleRad, nodePos[i], dir, distBetween);
        hitInfo rayBoxCheck = rayBoxListIntersect(boxes, boxW, boxH, nodePos[i], dir, distBetween);
        if (!circleListCheck.hit && !rayBoxCheck.hit){
            neighbors[i].add(j);
        }
      }
    }
...
void buildPRM(Vec2[] circleCenters, float[] circleRadii, ArrayList<Vec2> boxes, float boxW, float boxH){
  generateRandomNodes(circleCenters, circleRadii, boxes, boxW, boxH);
  connectNeighbors();
}
